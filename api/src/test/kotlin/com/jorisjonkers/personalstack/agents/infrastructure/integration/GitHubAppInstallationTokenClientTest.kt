...
class GitHubAppInstallationTokenClientTest {
    ...
    fun `mint returns null and never calls GitHub when disabled`() {
        ...
        assertThat(client.mint("git@github.com:JorisJonkers-dev/agents.git")).isNull()
    }
    ...
    fun `mint sends a valid RS256 app JWT and a repo-scoped token request, then returns the token`() {
        ...
        val minted = client.mint("git@github.com:JorisJonkers-dev/agents.git")
    ...
    fun `probe returns INSTALLED on a 200 installation lookup and never requests a token`() {
        ...
        assertThat(client.probe("git@github.com:JorisJonkers-dev/agents.git"))
    ...
    fun `probe returns UNKNOWN when disabled or the URL is unparseable, without any HTTP`() {
        ...
        assertThat(disabled.probe("git@github.com:JorisJonkers-dev/agents.git"))
            .isEqualTo(GithubAppInstallationState.UNKNOWN)
