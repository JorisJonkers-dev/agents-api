...
class WorkspaceRepositoryFlowIntegrationTest : IntegrationTestBase() {
    @Autowired
    ...
    @Test
    fun `project workspace creation surfaces primary and project repository extras in detail`() {
        val unique = unique()
    ...
    @Test
    fun `detaching the primary repository returns the validation error`() {
        val unique = unique()
        val primaryId = createRepository("primary-$unique")
        val workspaceId = createWorkspace(name = "workspace-$unique", repositoryId = primaryId)
    ...
    private fun unique(): String = UUID.randomUUID().toString().take(8)
}
