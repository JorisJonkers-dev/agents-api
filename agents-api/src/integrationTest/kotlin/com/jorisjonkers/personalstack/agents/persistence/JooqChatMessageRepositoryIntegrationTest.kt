...
class JooqChatMessageRepositoryIntegrationTest : IntegrationTestBase() {
    @Autowired
    ...
    fun `save and findById round-trip`() {
        ...
        messages.save(m)
        val loaded = messages.findById(m.id)
        assertThat(loaded).isNotNull
        assertThat(loaded!!.body).isEqualTo("hello")
    }

    @Test
    fun `findAllBySessionIdOrderedByTime returns messages chronologically`() {
        val s = newSession()
    ...
    @Test
    fun `cascading delete removes messages when the session is removed`() {
        val s = newSession()
    ...
    @Test
    fun `deleteAllBySessionId removes the rows`() {
        val s = newSession()
        messages.save(newMessage(s.id))
        messages.save(newMessage(s.id))
